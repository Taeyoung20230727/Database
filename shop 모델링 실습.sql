...
insert into `order` set `orderId` = 'c103',`orderproduct`=6, `orderCount`= 5, `orderDate`=Now();		# 실행X `ordercount`가 6번이 없는데 6번넣어서 X
...
join `product` as b on a.orderNo = b.prodNo
join `Customer`;

select `orderno`, `name`,`prodname`,`ordercount`,`orderdate` from `order` as a 
join `product` as b on a.orderNo = b.prodNo
join `Customer` where `name` = '김유신';


select
sum(`price`*`orderCount`) '총 주문금액'
from `order` as a
