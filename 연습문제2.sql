...
grant all privileges on `Bank`. * to 'qksrm5546'@'%';
grant all privileges on `shop`. * to 'qksrm5546'@'%';
flush privileges;
...
insert into `bank_account` values ('101-11-1001','S1','자유저축예금','370423-1000001','420000','2005-07-21');  
...
insert into `bank_transaction` values (9,'101-11-1003',3,0,'2022-08-21 04:26:52');  
...
#실습2-9
select distinct `a_item_dist`,`a_item_name` from `bank_account`;		# distinct - 중복제거

#실습2-10
select * from `bank_account` where `a_balance` >= 1000000 and `a_item_name` = '자유저축예금';

#실습2-11
select * from `bank_customer` where `c_addr` like '경기도 ___';			# like '~~~__' or '~~~ %' 문장중 글자포함한거 찾아내기
...
where `a_balance` = (select MAX(`a_balance`)
 from `bank_account` where `a_item_name` = '자유저축예금');
...
#실습2-20
select
count(if(`t_dist` = 1, 1, null)) as `입금 건수`,		#count문 안에 if문 들어갈수있음
